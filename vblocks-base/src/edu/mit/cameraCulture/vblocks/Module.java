...
/**
 * Dictates the blocks behavior. It represents the shape of the blocks. A sample
 * from the camera is given to the block through the <code>execute</code> method
 * as a Sample object.
 * 
...
public abstract class Module extends IntentService implements ModuleInterface{
	...
	@Retention(RetentionPolicy.RUNTIME)
	public @interface ImagePixelType {
		PixelEncoding encoding();
	}

	public enum ExecutionCode {
		NONE, EXIT_IMMEDIATELY, FINISH_CYCLE_AND_EXIT
	}
	...
	public static HashMap<String, Class> getVariables(Class classType) {
		...
		try {
			Annotation[] annotations = classType.getDeclaredMethod("execute",
					Sample.class).getAnnotations();
			if (annotations != null) {
	...
	private static void addAllToMap(HashMap<String, Class> map,
			Annotation annotation, Class type) {
		try {
			String[] names = (String[]) annotation.annotationType()
					.getMethod("vars").invoke(annotation);
			for (String name : names) {
				map.put(name, type);
			}
		} catch (Exception e) {
			Log.e("VisionBlocks",
					"If block: incorrect annotation for execute method ");
		}
	}
