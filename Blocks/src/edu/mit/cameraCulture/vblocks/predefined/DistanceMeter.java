...
public class DistanceMeter extends Module implements SensorEventListener{
	class CrossView extends View {
	    ...
	    protected void onDraw(Canvas canvas){
	    	...
	    	canvas.drawLine(this.getWidth()/2, 0, this.getWidth()/2, this.getHeight(), p);
	    }
	}
	...
	public void onCreate(EngineActivity context) {
        ...
        mSensorManager.registerListener(this, mSensorManager.getDefaultSensor(Sensor.TYPE_ACCELEROMETER), SensorManager.SENSOR_DELAY_NORMAL);
  	  	...
  	  	RelativeLayout.LayoutParams lp = new RelativeLayout.LayoutParams( (int) (metrics.density * 30 + 0.5f), (int) (metrics.density * 30 + 0.5f));
	  	...
	  	layout.addView(mSpeedLabel,lp);
  	  	...
  	  	lp = new RelativeLayout.LayoutParams( (int) (metrics.density * 100 + 0.5f), (int) (metrics.density * 60 + 0.5f));
			...
			public void onClick(View v) {
				if(mInitialOrientation == null){
					...
					System.arraycopy( mOrientation, 0, mInitialOrientation, 0, mOrientation.length );
					...
					// compute speed 
					double startDistance = 1.5/Math.tan((Math.PI/2) - mInitialOrientation[2]);
					double endDistance = 1.5/Math.tan((Math.PI/2) - mOrientation[2]);
					
					double angle = Math.max(mInitialOrientation[0], mOrientation[0]) - Math.min(mInitialOrientation[0], mOrientation[0]);
					
					double size = (startDistance*startDistance) + (endDistance*endDistance) - (2*startDistance*endDistance*Math.cos(angle));
	...
	public void onSensorChanged(SensorEvent event) {
        ...
        if (mGeomagneticmatrix != null && mAccelerometervalues != null) {
        	SensorManager.getRotationMatrix(mR, null, mAccelerometervalues, mGeomagneticmatrix);
            SensorManager.getOrientation(mR, mOrientation);
            //double distance = Math.toDegrees(mOrientation[0])+","+Math.toDegrees(mOrientation[1])+","+Math.toDegrees(mOrientation[2]));
            mDistanceLabel.setText(distanceLabel + 
            		new DecimalFormat("#.##").format(
            		-1.5/Math.tan((Math.PI/2) - mOrientation[2])) + " m");
            //Log.d("Test", "             "+ new DecimalFormat("#.##").format()+ " m");
            ...
            if(mInitialOrientation != null)
            {
				...
				time /= 1000;
				double speed = size / time;
				mObjectDistanceLabel.setText("Distance: "+ new DecimalFormat("#.##").format(size) +" m");
				mSpeedLabel.setText("Speed: "+ new DecimalFormat("#.##").format(speed*3.6) +" km/h");
            }
        }
      }
