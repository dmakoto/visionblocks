...
public class OpticalFlow extends Module {
	...
	private ModuleTouchListener mListener;
	...
	class BorderView extends View {
		...
		public BorderView(Context context) {
			...
			mListener = new ModuleTouchListener() {

				@Override
				public boolean onSingleTapConfirmed(MotionEvent arg0) {
					// TODO Auto-generated method stub
					Log.d("OPTICAL FLOW", "On touch");
					return false;
				}
		...
		protected void onDraw(Canvas canvas) {
			...
			canvas.drawLines(new float[] { rect.left, rect.top, rect.right,
					rect.top, rect.right, rect.top, rect.right, rect.bottom,
					rect.right, rect.bottom, rect.left, rect.bottom, rect.left,
					rect.bottom, rect.left, rect.top }, 0, 16,
					isValid ? pTracked : p);
	...
	public ExecutionCode execute(Sample image) {
		...
		if (mArea.isValid()) {
			...
			Mat mYUV_Mat = new Mat(image.getHeight(), image.getWidth(),
					CvType.CV_8UC4);
			Imgproc.cvtColor(image.getRgbMat(), mYUV_Mat,
					Imgproc.COLOR_RGB2YUV_I420);
	...
	private native int processImage(byte[] data, int width, int height,
			float[] rect);
	...
	public static final Parcelable.Creator<OpticalFlow> CREATOR = new Parcelable.Creator<OpticalFlow>() {
